...
POWERLINES_DEST_DIR = Path('data/sleeves/sleeves_powerlines_v2/')
ALL_DEST_DIR = Path('data/sleeves/sleeves_all_v2/')
NUM_IMAGES_TO_ADD = 530
    ...
    for img_path, json_path in file_pairs:
        ...
        if clear_labels:
            try:
                ...
                print(
                    f"Could not process or clear labels for {json_path.name}: {e}")
        else:
...
def main():
    ...
    if len(with_powerlines) < NUM_IMAGES_TO_ADD:
        print(
            f"Warning: Not enough images with powerlines ({len(with_powerlines)}) to meet the requirement of {NUM_IMAGES_TO_ADD}.")
    ...
    print(
        f"Added {len(powerlines_to_copy)} images with powerlines (labels removed).")
