...
def fix_dataset_structure(source_dir, target_dir, dry_run=False):
    ...
    for split_name, split_info in source_structure.items():
        print(
            f"  {split_name}: {split_info['image_count']} images, {split_info['label_count']} labels")
        total_source_images += split_info['image_count']
        total_source_labels += split_info['label_count']

    print(
        f"  Total: {total_source_images} images, {total_source_labels} labels")

    # Analyze target structure (if it exists)
    target_structure = get_split_structure(
        target_dir) if os.path.exists(target_dir) else {}
    ...
    if target_structure:
        ...
        print(
            f"  Total: {total_target_images} images, {total_target_labels} labels")
    ...
    print(
        f"  Total: {total_target_images_after} images, {total_target_labels_after} labels")

    # Report results
    print("\nCopy operation complete:")
    print(
        f"  Copied {copy_count['images']} images, {copy_count['labels']} labels")
    ...
    if total_source_images != total_target_images_after or total_source_labels != total_target_labels_after:
        ...
        print(
            f"  Source: {total_source_images} images, {total_source_labels} labels")
        print(
            f"  Target: {total_target_images_after} images, {total_target_labels_after} labels")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Fix dataset structure by properly copying files')
